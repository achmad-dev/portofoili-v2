...
impl AiService {
    ...
    pub async fn get_messages(&self, page: i64, limit: i64) -> Result<Vec<crate::domain::entities::ChatMessage>, AppError> {
        let offset = (page - 1) * limit;
        self.chat_repo.get_paginated_chats(offset, limit).await
    }

    pub async fn generate_response(&self, ip: String, prompt: String, local_tx: mpsc::Sender<AiEvent>) {
        ...
        let _ = broadcast_event(AiEvent::Thinking("Checking security rules and rate limits...".to_string())).await;
        ...
        if let Ok(allowed) = self.chat_repo.check_global_rate_limit().await {
            if !allowed {
                let _ = broadcast_event(AiEvent::Error("Global rate limit exceeded".to_string())).await;
                return;
            }
        } else {
            let _ = broadcast_event(AiEvent::Error("Internal database error".to_string())).await;
        ...
        // Input Guardrail
        let _ = broadcast_event(AiEvent::Thinking("Evaluating input guardrails...".to_string())).await;
        match self.ai_provider.evaluate_guardrail(&prompt).await {
            ...
            Ok(false) => {
                ...
                let _ = broadcast_event(AiEvent::Error("Input rejected by safety guidelines.".to_string())).await;
                return;
            }
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to evaluate safety guidelines.".to_string())).await;
                return;
            }
        }

        // RAG Context
        let _ = broadcast_event(AiEvent::Thinking("Embedding query and searching knowledge base...".to_string())).await;
        let query_embedding = match self.ai_provider.get_embedding(&prompt).await {
            Ok(emb) => emb,
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to generate embedding.".to_string())).await;
                return;
            }
        };
        ...
        let similar_docs = match self.chat_repo.get_similar_documents(query_embedding, 3).await {
            Ok(docs) => docs,
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to search knowledge base.".to_string())).await;
                return;
            }
        };
        ...
        // Memory Context
        let _ = broadcast_event(AiEvent::Thinking("Fetching global chat history...".to_string())).await;
        let previous_chats = match self.chat_repo.get_recent_chats(5).await {
            Ok(chats) => chats,
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to fetch chat history.".to_string())).await;
                return;
            }
        };
        ...
        let response_text = match self.ai_provider.generate_content(&final_prompt).await {
            Ok(res) => res,
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to generate response.".to_string())).await;
                return;
            }
        };
        ...
        match self.ai_provider.evaluate_guardrail(&response_text).await {
            ...
            Ok(false) => {
                ...
                let _ = broadcast_event(AiEvent::Error("Generated response rejected by safety guidelines.".to_string())).await;
                return;
            }
            Err(_) => {
                let _ = broadcast_event(AiEvent::Error("Failed to verify output safety.".to_string())).await;
                return;
            }
        }
