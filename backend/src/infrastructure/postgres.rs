...
impl ChatRepository for SupabaseRepository {
    async fn check_ip_rate_limit(&self, ip: &str) -> Result<bool, AppError> {
        ...
        let count: (i64,) = sqlx::query_as(
            "SELECT count(*) FROM chats WHERE ip_address = $1 AND DATE(created_at) = $2",
        )
        .bind(ip)
        .bind(today)
        .fetch_one(&self.pool)
    ...
    async fn check_global_rate_limit(&self) -> Result<bool, AppError> {
        let today = chrono::Utc::now().date_naive();
        let count: (i64,) =
            sqlx::query_as("SELECT count(*) FROM chats WHERE DATE(created_at) = $1")
                .bind(today)
                .fetch_one(&self.pool)
                .await?;
    ...
    async fn get_similar_documents(
        ...
        let similar_docs: Vec<(String,)> =
            sqlx::query_as("SELECT content FROM match_documents($1::vector, 0.5, $2)")
                .bind(embedding)
                .bind(limit as i32)
                .fetch_all(&self.pool)
                .await?;
        ...
        Ok(similar_docs
            .into_iter()
            .map(|d| SimilarDocument { content: d.0 })
            .collect())
    }
