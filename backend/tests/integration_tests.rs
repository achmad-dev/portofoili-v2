...
async fn test_ai_generate_success_with_hmac() {
    ...
    mock_repo
        .expect_check_ip_rate_limit()
        .returning(|_| Ok(true));
    mock_repo
        .expect_check_global_rate_limit()
        .returning(|| Ok(true));

    mock_ai.expect_evaluate_guardrail().returning(|_| Ok(true));

    mock_ai
        .expect_get_embedding()
        .returning(|_| Ok(Vector::from(vec![0.0; 768])));

    mock_repo
        .expect_get_similar_documents()
        .returning(|_, _| Ok(vec![]));
    mock_repo
        .expect_get_recent_chats()
        .returning(|_| Ok(vec![]));

    mock_ai
        .expect_generate_content()
        .returning(|_| Ok("Mocked AI response".to_string()));
...
async fn test_get_messages_pagination() {
    ...
    mock_repo
        .expect_get_paginated_chats()
        .returning(|offset, limit| {
            assert_eq!(offset, 10);
            assert_eq!(limit, 10);

            Ok(vec![backend::domain::entities::ChatMessage {
    ...
    let req = test::TestRequest::get()
        .uri("/api/ai/messages?page=2&limit=10")
        .to_request();
    let resp = test::call_service(&app, req).await;
