...
export const TerminalBuffer = () => {
  ...
  const handleCommand = (cmd) => {
    ...
    switch (command) {
      ...
      case 'whoami':
        ...
        break;
      case 'ls':
        response = 'content/  projects/  blog/  cybersecurity/  config/  copilot.chat';
        break;
      case 'clear':
        setHistory([]);
        setInput('');
      ...
      case 'nmap':
        if (args.length > 1) {
           ...
           response = 'nmap: Missing target specification.\nUsage: nmap [Scan Type(s)] [Options] {target specification}';
        }
        break;
      case '':
        break;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!input && input.trim() === '') {
       setHistory(prev => [...prev, { type: 'input', text: `guest@portfolio:~$ ` }]);
       return;
    }
    handleCommand(input);
    setInput('');
  ...
  return (
    <div className="flex flex-col h-full font-mono p-4 text-sm bg-catppuccin-base text-catppuccin-text">
      <div className="flex-1 overflow-y-auto space-y-1 pb-4 custom-scrollbar pr-2">
        {history.map((line, i) => (
          <div key={i} className={`whitespace-pre-wrap ${line.type === 'input' ? 'text-catppuccin-green font-bold' : 'text-catppuccin-subtext0'}`}>
            {line.text}
          </div>
        ))}
        <div ref={bottomRef} />
      ...
      <form onSubmit={handleSubmit} className="mt-2 flex gap-2 items-center flex-shrink-0">
        <span className="text-catppuccin-green font-bold">guest@portfolio:~$</span>
        <input
          type="text"
          value={input}
